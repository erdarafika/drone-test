...
<script>
import { fetchProvinceList, createProvince, updateProvince, deleteProvince, updateStatusProvince } from '@/api/location'
...
export default {
  ...
  methods: {
    handleUpdateStatus(row) {
      ...
      updateStatusProvince(payload).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateProvince(this.tempUpdate, this.id, this.tempUpdate.objectId).then((response) => {
            this.dialogFormVisible = false
            if (response.status_code >= 200 && response.status_code <= 300) {
              this.successNotifier()
              this.getList()
    ...
    handleDelete(row) {
      ...
      const deleteCallback = () => {
        deleteProvince({ countryId: this.id, id: row.id }).then(() => {
          this.dialogFormVisible = false
          this.successNotifier()
          this.getList()
        })
