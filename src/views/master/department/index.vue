  ...
  el-dialog(:title='getDialogHeader(dialogStatus)', :visible.sync='dialogFormVisible')
    el-form(ref='dataForm', :rules='rules', :model='temp', label-position='left', label-width='200px', style='width: 80%; margin-left:50px;')
      el-form-item(:label="$t('department.code')", prop='code')
        el-input(v-model='temp.code', type='textarea', :autosize='{ minRows: 2, maxRows: 4}' )
      ...
      el-form-item(:label="$t('department.description')", prop='description')
        el-input(v-model='temp.description', type='textarea', :autosize='{ minRows: 4, maxRows: 4}' )
  ...
  computed: {
    rules() {
      return {
        ...
        description: [{ validator: alphanumericValidator }]
      }
    },
    filterredList() {
